...
class InternVL3UITarsComboMethod:      
    ...
    def load_model(self, model_name_or_path=None):      
        self.debug_print(f"使用OpenRouter API调用模型: {self.internvl3_model} + {self.uitars_model}")      
    ...
    def ground_with_internvl3(self, instruction, image):      
...
Where x and y are floating-point numbers between 0 and 1."""      
    ...
    def refine_with_uitars(self, instruction, image, internvl3_point, internvl3_response):      
        if internvl3_point is None:      
...
**Previous Model's Prediction**: [{internvl3_point[0]:.4f}, {internvl3_point[1]:.4f}]      
...
Coordinates must be normalized [0,1] range."""      
    ...
    def ground_only_positive(self, instruction, image):      
        ...
        # 第一层: InternVL3初检  
        internvl3_point, internvl3_response = self.ground_with_internvl3(instruction, image)      
            
        if internvl3_point is not None:      
            time.sleep(1)      
            
        # 第二层: UI-TARS修正  
        final_point, uitars_response = self.refine_with_uitars(      
            instruction, image, internvl3_point, internvl3_response      
        )      
            
        # 如果UI-TARS也失败,回退到InternVL3结果  
        if final_point is None and internvl3_point is not None:      
            self.debug_print("UI-TARS修正失败,使用InternVL3原始结果")      
            final_point = internvl3_point      
