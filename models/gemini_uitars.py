...
class GeminiUITarsComboMethod:    
    """双层组合模型: Gemini初检 + UI-TARS修正"""    
    ...
    def ground_with_gemini(self, instruction, image):    
        """使用Gemini进行初检 - 使用GeminiOnlyMethod的prompt"""    
        self.debug_print("=== Gemini初检 ===")    
    
        # 使用与GeminiOnlyMethod相同的prompt    
...
- Consider the element's visual appearance (color, size, shape) if mentioned in the instruction"""    
    ...
    def refine_with_uitars(self, instruction, image, gemini_point, gemini_response):    
        """使用UI-TARS进行修正"""    
        if gemini_point is None:    
            # 如果Gemini未找到目标,让UI-TARS独立检测    
            self.debug_print("=== UI-TARS独立检测 ===")    
...
x, y must be normalized (0-1 range)."""    
        ...
        # 使用Gemini的结果作为参考,让UI-TARS验证和修正    
        self.debug_print("=== UI-TARS修正 ===")    
        prompt = f"""Task: {instruction}  
  
Previous detection: x={gemini_point[0]:.4f}, y={gemini_point[1]:.4f}  
  
Verify this coordinate. If correct, return same. If incorrect, return corrected coordinate.  
  
Output format: {{"coordinate": [x, y], "confidence": "high/medium/low", "reasoning": "brief explanation"}}  
...
x, y must be normalized (0-1 range)."""  
        ...
        # 解析UI-TARS响应  
        image_size = (image.width, image.height)  
        refined_point = self.parse_uitars_response(response, image_size)  
    ...
    def ground_only_positive(self, instruction, image):  
        ...
        # 第一层: Gemini初检  
        gemini_point, gemini_response = self.ground_with_gemini(instruction, image)  
          
        # 添加延迟避免速率限制  
        if gemini_point is not None:  
            time.sleep(1)  
          
        # 第二层: UI-TARS修正  
        final_point, uitars_response = self.refine_with_uitars(  
            instruction, image, gemini_point, gemini_response  
        )  
          
        # 如果UI-TARS也失败,回退到Gemini结果  
        if final_point is None and gemini_point is not None:  
            self.debug_print("UI-TARS修正失败,使用Gemini原始结果")  
            final_point = gemini_point  
